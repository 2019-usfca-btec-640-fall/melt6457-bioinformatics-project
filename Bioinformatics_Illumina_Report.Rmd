---
title: 'Presence of the proteobacteria *Bradyrhizobium*, *Pseudomonas*, and *Burkholderia* indicate a healthy microbiome community at the UNDE site which could be strongly influenced by acidic Sphagnum Bogs'
author: "Kory Melton"
...
##########################################
# Make plot ID's more readable by taking
# substring of the plots with just the
# numbers
##########################################
...
# pull out genus (first word)
top_5_genus$genus <- gsub(" .*$", 
                          "", 
                          pattern = top_5_genus$Scientific.Name)

# pull out species (after first word)
top_5_genus$species <- gsub(".* ", 
                            "", 
                            pattern = top_5_genus$Scientific.Name)
```
...
Microscopic organisms are among the oldest and most prevalent lifeforms on earth. Accordingly, they play a vital role in the health of ecosystems by affecting processes like carbon dioxide and nitrogen cycling [@martiny]. As the worlds climate changes over time understanding how microbes influence the dynamics of ecosystems will be necessary to preserve the environment. Currently, the impacts of climate change on the biogeochemical cycling of microbes are not well understood [@barnard]. However, some research suggests microbes may be able to mediate carbon-cycle feedback to climate warming [@zhou]. 
...
The climate is generally humid, cool, and wet with no true dry season and receives approximately 114 inches of snow annually. The soil families include coarse, loamy, mixed, superactive, frigid, argic fragiaquods. Although, some of the soil on the site is poorly drained, which may cause acidic sphagnum bogs. This is especially troublesome since climate warming can promote the growth of peat mosses and compound this issue [@breeuwer2009decreased]. The presence of sphagnum is known to increase the amount of dissolved organic Nitrogen; reducing Nitrogen immobilization by soil microbes [@bragazza2013biogeochemical]. The competition for nutrients and changes caused by sphagnum bogs and climate warming, can short-circuit the Nitrogen cycle. In years where sphagnum bogs are prevalent one could expect lower abundances of soil microbes. In addition, ecosystems with a high abundance of nitrogen-fixing bacteria may be more suited to withstand the disruption caused by sphagnum bogs.
...
# put *genus* so the output for tables is italics
top_3_staph_rals_pro$scientific_name <-
  paste0("*",
         top_3_staph_rals_pro$scientific_name,
         "*")

# remove the *genus* for proteobacteria since it is not a genus
top_3_staph_rals_pro$scientific_name <-
  gsub("[*]Proteobacteria[*]",
       "Proteobacteria",
       top_3_staph_rals_pro$scientific_name)
...
``` {r kingdom-analysis}
# get information from the melted phyloseq on kingdom by
# collection year and collection month
...
# graph the number of each kingdom by collection year
kingdom_info %>%
...
**Figure 10.** Summary of Kingdom by year. The samples were primarily bacterial, however a small number of the samples were archaeal. In addition, this graph shows that almost 300 more samples were collected in 2016.

``` {r phyla-by-plot}
# list the top phyla in descending order of abundance
...
# keep only the top phyla for each plot
top_phyla_by_plot <- phyla_by_plot[!duplicated(phyla_by_plot$plotID), ]

# graph the top phyla by plotID with different colors for each phyla
top_phyla_by_plot %>%
  ggplot(aes(x = plotID,
             y = sum_abund,
             fill = Phylum)) +
...
# graph the abundances of the top 5 diverse phyla in a facet grid
...
# graph the abundance of genera of proteobacteria in a facet grid
# separated by months in 2016
# filter out month 10 because it does not have proteobacteria
melted_phyloseq %>%
...
``` {r richness-by-year}
# prune sample to only include months 5 and 8
...
``` {r richness-2016}
# prune the samples to only include the year 2016
