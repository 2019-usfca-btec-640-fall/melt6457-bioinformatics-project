...
downloadRawSequenceData <- function(dpid="", targetGene="16S", sites="all", startYrMo="YYYY-MM", endYrMo="YYYY-MM", 
  ...
  # Outputs: 1) data frame of raw data records of interest; 2) raw sequence data, if download=TRUE
  ...
  # checkFileSize - if TRUE, then user will be prompted to select y or n during metadata download. (options TRUE or FALSE)
  # download - indicates whether sequence data should be automatically downloaded (options TRUE or FALSE)
  ...
  print("Grabbing metadata from the NEON Data Portal. This may take a few moments")
  dat <- loadByProduct(dpID=dpid, site=sites, startdate=startYrMo, enddate=endYrMo,
  ...
  for(i in 1:length(u.urls)) {
    ...
    fileSize[i] <- round(as.numeric(httr::headers(response)[["Content-Length"]])/1048576, 1)   # convert bytes to MB
  }
  # Report number and sizes of files to download #
  print(paste("There are", length(u.urls), "unique files to download, total space required:",
              sum(fileSize), "MB") )
  ...
  if(download==TRUE) {
    ...
    for(i in 1:length(u.urls)) {
      download.file(url=as.character(u.urls[i]), destfile = paste(outdir, fileNms[i], sep="/") ) 
    }  
  }
